...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber imu_sub = n.subscribe("imu/data", 50, imu_callback); //imu/data
  ...
  while(n.ok()){
    ...
    double linear_velocity = vel_x; //z
  ...
  //  double delta_theta = angular_velocity * imu_dt * angular_scale; //radians
    double delta_theta = angular_velocity * imu_dt; //zee
  ...
  //  double delta_x = (linear_velocity * cos(theta)) * vel_dt * linear_scale; //m
   // double delta_y = (linear_velocity * sin(theta)) * vel_dt * linear_scale; //m
    ...
    double delta_x = (linear_velocity * cos(theta)) * vel_dt;//z
    double delta_y = (linear_velocity * sin(theta)) * vel_dt;//z
    ...
    odom.twist.twist.linear.x = linear_velocity;  //z
    odom.twist.twist.linear.y = 0.0;  //z
